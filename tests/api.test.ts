...
const dataset: Dataset = {
  station: { name: 'Test station', latitude: -1.0916, longitude: 37.014, elevationM: 1500 },
  observations: [],
  hours: [],
  sources: [],
...
beforeEach(async () => {
  ...
  const instance = createApp({
    ...
    forecast: async (latitude, longitude) => ({
...
describe('authentication and tenant security', () => {
  ...
  it('rejects weak credentials, duplicate accounts, wrong passwords and expired sessions', async () => {
    ...
    expect(
      (await post('/api/auth/login', { email: 'unknown@example.test', password })).status,
    ).toBe(401);
    db.prepare('UPDATE sessions SET expires_at=0').run();
    expect((await get('/api/workspace', cookie)).status).toBe(401);
    expect(
      (await post('/api/auth/login', { email: 'operator@example.test', password })).status,
    ).toBe(200);
  });
  it('isolates demo workspaces without shared users or invented emails', async () => {
    const a = await post('/api/auth/demo', {});
    const b = await post('/api/auth/demo', {});
    const ua = (await a.json()).user;
    const ub = (await b.json()).user;
    expect(ua.id).not.toBe(ub.id);
    expect(ua.email).toBe('');
    expect(ub.email).toBe('');
    const ca = cookieFrom(a),
      cb = cookieFrom(b);
  ...
  it('blocks origin spoofing, missing custom CSRF header and unsupported content types', async () => {
    expect((await post('/api/auth/demo', {}, '', { Origin: 'https://attacker.test' })).status).toBe(
      403,
    );
    expect((await post('/api/auth/demo', {}, '', { 'X-Kavu-Request': '' })).status).toBe(403);
    expect((await post('/api/auth/demo', {}, '', { 'Content-Type': 'text/plain' })).status).toBe(
      415,
    );
    expect((await post('/api/auth/demo', {}, '', { Origin: 'null' })).status).toBe(403);
  });
  it('refuses ephemeral SQLite when a production Cloud Run service omits Firestore configuration', () => {
    vi.stubEnv('K_SERVICE', 'kavu-api-fixture');
    vi.stubEnv('DATA_STORE', 'sqlite');
    try {
      expect(() => createApp({ production: true, origin: 'https://kavu.example.test' })).toThrow(
        /Cloud Run requires DATA_STORE=firestore/,
      );
    } finally {
      vi.unstubAllEnvs();
    }
  ...
  it('requires HTTPS production origins and sets host-only secure cookies', async () => {
    expect(() => createApp({ db, production: true, origin: 'http://example.test' })).toThrow(
      /HTTPS/,
    );
    expect(() =>
      createApp({
        db,
        production: true,
        origin: 'https://kavu.example.test',
        additionalOrigins: ['https://secondary.example.test/unsafe'],
      }),
    ...
    const secure = createApp({
      ...
      production: true,
      origin: 'https://kavu.example.test',
      additionalOrigins: ['https://secondary.example.test'],
      dataset,
      rateLimit: false,
      distPath: '/does-not-exist',
    });
    const listener = await new Promise<Server>((resolve) => {
      const instance = secure.app.listen(0, '127.0.0.1', () => resolve(instance));
    });
    try {
      const origin = `http://127.0.0.1:${(listener.address() as AddressInfo).port}`;
      const response = await fetch(`${origin}/api/auth/demo`, {
        method: 'POST',
        headers: { ...headers, Origin: 'https://kavu.example.test' },
        body: '{}',
      });
      ...
      const secondary = await fetch(`${origin}/api/auth/demo`, {
        method: 'POST',
        headers: { ...headers, Origin: 'https://secondary.example.test' },
        body: '{}',
      });
      expect(secondary.status).toBe(201);
      expect(secondary.headers.get('cache-control')).toBe('private, no-store');
      const foreign = await fetch(`${origin}/api/auth/demo`, {
        method: 'POST',
  ...
  it('bounds concurrent account sessions and keeps the newest one valid', async () => {
    ...
    for (let i = 0; i < 22; i++)
      latest = cookieFrom(
        await post('/api/auth/login', { email: 'operator@example.test', password }),
      );
  ...
  it('rate-limits repeated authentication requests', async () => {
    ...
    const listener = await new Promise<Server>((resolve) => {
      const instance = limited.app.listen(0, '127.0.0.1', () => resolve(instance));
    });
    try {
      const origin = `http://127.0.0.1:${(listener.address() as AddressInfo).port}`;
  ...
  it('changes passwords and revokes every earlier session', async () => {
    ...
    const changed = await post(
      '/api/auth/password',
      { currentPassword: password, newPassword: 'another-strong-passphrase' },
      cookie,
    );
...
describe('atomic workspace API', () => {
  it('enforces optimistic concurrency and ownership and rejects reset for real accounts', async () => {
    ...
    expect(
      (
        await post(
          ...
          {
            revision: 0,
            command: { type: 'batch.measure', batchId: w.batches[0].id, moisturePct: 12, note: '' },
          },
          b.cookie,
        )
      ).status,
    ...
    expect(
      (await post('/api/commands', { revision: 0, command: { type: 'demo.reset' } }, b.cookie))
        .status,
    ).toBe(403);
  ...
  it('allows exactly one winner for concurrent writes with the same revision', async () => {
    ...
    const responses = await Promise.all([
      post('/api/commands', { revision: 0, command }, cookie),
      post('/api/commands', { revision: 0, command }, cookie),
    ]);
  ...
  it('returns safe errors for malformed requests and denies anonymous writes', async () => {
    expect(
      (await post('/api/commands', { revision: 0, command: { type: 'demo.reset' } })).status,
    ).toBe(401);
    const { cookie } = await register();
    expect(
      (await post('/api/commands', { revision: -1, command: { type: 'demo.reset' } }, cookie))
        .status,
    ).toBe(400);
    const invalid = await fetch(`${base}/api/auth/demo`, {
      method: 'POST',
      headers,
      body: '{broken',
    });
  ...
  it('serves SPA deep links while preserving API and missing-asset 404 responses', async () => {
    ...
    try {
      ...
      expect(await page.text()).toContain('Kavu test shell');
      const missingAsset = await fetch(`${origin}/missing.js`);
      expect(missingAsset.status).toBe(404);
      const missingApi = await fetch(`${origin}/api/not-real`, {
  ...
  it('serves provenance data publicly but requires auth and valid coordinates for live forecasts', async () => {
    ...
    expect((await get('/api/forecast?latitude=-1&latitude=0&longitude=37', cookie)).status).toBe(
      400,
    );
...
describe('forecast parsing', () => {
            ...
            timeseries: [
              {
                ...
                data: {
                  instant: {
                    details: { air_temperature: 28, relative_humidity: 50, wind_speed: 2 },
                  },
  ...
  it('does not treat absent rainfall or impossible humidity as safe drying evidence', () => {
    const forecast = parseForecast(
      {
        properties: {
          meta: { updated_at: '2026-09-22T00:00:00Z' },
          timeseries: [
            {
              time: '2026-09-22T06:00:00Z',
              data: {
                instant: {
                  details: { air_temperature: 28, relative_humidity: 150, wind_speed: 2 },
                },
              },
            },
          ],
