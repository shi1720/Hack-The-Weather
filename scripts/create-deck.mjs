...
const releaseVerified=process.env.KAVU_CLOUD_VERIFIED==='true';
...
text(s,'Shivam Gupta\nFounder and product lead',80,577,620,64,22,colors.light);
text(s,'kavu-drying.web.app',80,653,1050,32,23,colors.gold,true);
...
s=slide(colors.forest);heading(s,'Conduit changes the work plan',true);
text(s,quality.uniqueRows.toLocaleString('en-US'),76,190,580,92,76,colors.ivory,true,'Manrope');
text(s,'unique observations with traceable sources',79,294,1040,42,27,colors.light);
text(s,'2,700 kg',77,406,590,82,64,colors.gold,true,'Manrope');
text(s,'12 September: outdoor allocation',80,499,570,46,26,colors.light);
text(s,'0 kg',743,406,440,82,64,colors.gold,true,'Manrope');
text(s,'Humid day or missing evidence',747,499,450,72,26,colors.light);
text(s,'Same example lots, deadlines and capacity. Retrospective comparison.',80,597,1080,40,24,colors.light);foot(s,3,true);
note(s,'Derived source: data/processed/quality-report.json and docs/evaluation.md. Input21,189 rows,18,364 unique,2,825 overlapping rows removed. Observations span28August to15September2026UTC. Longestgap144.029hours. The controlled evaluation holds inventory, deadline, capacity and September12 planning date fixed and maps comparison weather timestamps to the same hour on that date. September12 allocates2700kg; September15 humid and September8 gap allocate0. This demonstrates functional data dependence and does not estimate forecast accuracy or field outcomes. Original source folder https://drive.google.com/drive/folders/1KDoCh8vss7nv_B6SuVBlQQssjSh1yaBg . Attribution https://3d-fewsnet.icdp.ucar.edu/instruments/61 . Data redistribution and commercial terms need owner confirmation.');
...
let shot=path.join(out,'overview-release.png');try{await fs.access(shot)}catch{shot=path.join(root,'output/screenshots/overview.png')}
...
s=slide();heading(s,'Private accounts and cloud records');
text(s,'Hosted application',74,207,690,53,36,colors.forest,true,'Manrope');
text(s,'Firebase Hosting',78,294,620,52,31,colors.forest,true);
text(s,'React and TypeScript interface',78,345,620,43,26,colors.muted);
text(s,'Cloud Run API with Firestore',78,426,640,55,31,colors.forest,true);
text(s,'Private accounts and persistent lot records',78,479,630,75,26,colors.muted);
text(s,'Self-hosting',834,211,380,50,34,colors.forest,true,'Manrope');
text(s,'Express and SQLite\nDocker deployment\nShared decision rules',838,310,354,191,29,colors.muted);
text(s,releaseVerified?'kavu-drying.web.app':'Cloud release awaiting final verification',79,601,1110,40,25,colors.forest,true);foot(s,5);
note(s,'Architecture source: project implementation and deployment configuration. Firebase Hosting serves the React client, Cloud Run runs the Express API and a dedicated Firestore database persists cloud accounts, sessions and workspaces. SQLite remains the local self-hosting backend. Deployment verification status: '+(releaseVerified?'implementation owner confirmed hosted checks':'pending; do not describe launch as verified until hosted tests pass')+'. The current workspace has one account owner, not multi-operator role assignments. Demo workspaces are separate. Optional MET Norway forecast is separately attributed; historic Conduit observations do not calibrate it. Product direction Shivam Gupta; Codex-assisted engineering.');

s=slide(colors.gold);heading(s,'The economics of one example');
text(s,'KSh 2,176',72,190,1120,130,105,colors.forest,true,'Manrope');
text(s,'Drying-tariff equivalent',80,342,1100,58,40,colors.forest,true);
text(s,'1.8 tonnes × 3.2 percentage points × KSh 377.80',80,445,1100,56,31,colors.forest);
text(s,'Mavuno A-01 example: 18.2% to 15.0%\nActual savings need labour, transport and alternative cost records.',82,543,1090,80,27,colors.ink);foot(s,6);
note(s,'NCPB published reference tariff: KSh377.80 per tonne per moisture percentage point, https://ncpb.co.ke/drying/ . Mavuno A-01 uses example intake mass1800kg, intake18.2%, follow-up15.0%. Calculation1.8x3.2x377.8=2176.128, displayed rounded KSh2176. Inputs are demonstration records, not field observations. This reference equivalent excludes actual labour, transport and handling, can differ from depot charges, and does not establish verified savings or causal impact. The ledger uses intake and latest moisture gap relative to target, rather than summing every downward reading.');
...
text(s,'The daily alternative is a weather\nreport and a notebook. Renewal\ndepends on easier handovers.',76,398,552,137,29,colors.muted);
...
text(s,'Price to test in a pilot\nTrack support effort and renewal',760,433,445,111,28,colors.forest);
text(s,'Start with yards that already use moisture meters. Grow through useful lot and cost histories.',76,582,1125,66,24,colors.muted);foot(s,7);
...
text(s,'A supervised pilot\nwith up to three yards',74,208,1090,149,59,colors.ivory,true,'Manrope');
text(s,'Observe the operator, then run in shadow mode.\nMeasure handovers, moisture checks and actual costs.\nTest continued use and willingness to renew.',78,416,1090,159,30,colors.light);
...
async function thumbnail(width,height,filename){
 const thumb=Presentation.create({slideSize:{width,height}});const ts=thumb.slides.add();ts.background.fill=colors.forest;
 const scale=width/1280;
 text(ts,'kavu.',56*scale,45*scale,500*scale,117*scale,96*scale,colors.ivory,true,'Manrope');
 text(ts,'Weather into\nmaize-drying jobs',59*scale,207*scale,515*scale,228*scale,55*scale,colors.ivory,true,'Manrope');
 text(ts,'Real Conduit data.\nA clear next action.',61*scale,500*scale,450*scale,92*scale,30*scale,colors.gold);
 text(ts,'Hack The Weather 2026',61*scale,height-57*scale,590*scale,34*scale,22*scale,colors.light);
 ts.images.add({blob:new Uint8Array(await fs.readFile(shot)),contentType:'image/png',alt:'Actual Kavu application in historical replay with example batches',fit:'contain',position:{left:594*scale,top:143*scale,width:646*scale,height:456*scale}});
 const png=await thumb.export({slide:ts,format:'png',scale:1});await fs.writeFile(path.join(root,'output',filename),new Uint8Array(await png.arrayBuffer()));
}
await thumbnail(1280,720,'kavu-youtube-thumbnail.png');
await thumbnail(1200,800,'kavu-devpost-thumbnail.png');
